...
async function seleccionarConversacion(entorno:EntornoSmokeAislado,proveedor:ProveedorDurable,id:string){const c=configs[proveedor],sesion=entorno.env.CAPI_WEBBRIDGE_SESSION!;const inventario=await webbridge(sesion,'list_tabs').catch(()=>({tabs:[]}));const tab=(inventario.tabs??[]).find((t:any)=>String(t.url??'').includes(c.host)&&String(t.url??'').includes(id));if(tab?.url)await webbridge(sesion,'find_tab',{url:tab.url,active:false});else await webbridge(sesion,'navigate',{url:c.url(id),newTab:true,group_title:'Smoke durable CAPI'});for(let i=0;i<30;i++){const d=await webbridge(sesion,'evaluate',{code:`location.pathname.includes(${JSON.stringify(id)})`}).catch(()=>({value:false}));if(d.value)return;await dormir(500)}throw new Error('La conversación no quedó activa en WebBridge')}
