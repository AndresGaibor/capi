...
export const comandoDiagnosticoPagina = defineCommand({
  ...
  args: { proveedor: { type: "string", alias: "p", default: "deepseek" }, output: { type: "string", default: "json" } },
  async run({ args }) {
    try { consola.log(JSON.stringify(await crearAplicacion().diagnosticarPagina.ejecutar(String(args.proveedor)))); }
