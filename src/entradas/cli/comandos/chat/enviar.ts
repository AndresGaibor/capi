...
export const argumentosChat = {
  ...
  razonamiento: { type: "boolean" as const }, busqueda: { type: "boolean" as const }, archivo: { type: "string" as const, alias: "f" },
...
export async function ejecutarChat(args: Record<string, unknown>): Promise<void> {
  ...
  if (args.dryRun) {
    const plan = { project: proyecto, provider: proveedor, model: modelo ?? "default", selection: args.nueva ? { motivo: "nueva" } : seleccion, fallback: Boolean(args.fallback), actions: ["seleccionar conversación", "adquirir lease", "navegar proveedor", "enviar prompt", "registrar conversación"] };
