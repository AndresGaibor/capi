...
export class EnviarMensajeConContexto {
  ...
  async *ejecutar(proveedorId: string, peticion: PeticionChat): AsyncGenerator<EventoStreaming> {
    ...
    try {
      const intentos = peticion.permitirFallback === false ? construirIntentosRecuperacion(proveedorId, peticion.modelo).slice(0, 1) : construirIntentosRecuperacion(proveedorId, peticion.modelo);
