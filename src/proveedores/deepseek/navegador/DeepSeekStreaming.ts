  ...
  async *observar(): AsyncGenerator<EventoStreaming> {
    ...
    for(let i=0;i<1800;i++){
      ...
      if(i>10 && i%5===0){
        ...
        if(api.contenido.startsWith(lr)&&api.contenido.length>lr.length){yield{tipo:'respuesta',contenido:api.contenido.slice(lr.length)};lr=api.contenido;sin=0;}
        if(api.terminado&&api.contenido){yield{tipo:'fin'};return;}
      }
