...
export const scriptEstadoStreamingDeepSeek = () => `(() => {
  ...
  const promptActual=String(window.__capiDeepSeekEnvio?.prompt||'');
  let raiz=document.querySelector('main')||document;
  if(promptActual&&input){
    let nodo=input.parentElement;
    while(nodo&&nodo!==document.body){
      if(String(nodo.innerText||nodo.textContent||'').includes(promptActual)){raiz=nodo;break;}
  ...
  const ultimo=asistentesRol.at(-1)||asistentesClase.at(-1)||null;
  const thinkNodes=ultimo?[...ultimo.querySelectorAll('[data-testid*="thinking" i],[class*="thinking"],[class*="reasoning"]')]:[];
  const domNode=ultimo?(__capiDom.primeroVisible(S.respuestaCandidatos,ultimo)||ultimo.querySelector(S.respuestaCandidatos.join(','))):null;
  ...
  const warning=__capiDom.primeroVisible(['[role="alert"]','.ds-button--warning','[data-testid*="error" i]'],ultimo||document);
  const toolbar=ultimo?(__capiDom.primeroVisible(['[role="toolbar"]','button[aria-label*="copy" i]','button[aria-label*="copiar" i]'],ultimo)||ultimo.querySelector('[role="toolbar"],button[aria-label*="copy" i],button[aria-label*="copiar" i]')):null;
