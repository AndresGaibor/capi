...
test("Qwen informa estrategia DOM", async () => {
  ...
  const t:any = { evaluar: async (code:string) => ({ value: code.includes("const texto") ? { visible:true, procesando:false, error:"" } : code.includes("data-menu-id") || code.includes("new File") ? { ok:true } : code.includes("Eliminar archivo") ? 0 : true }) };
