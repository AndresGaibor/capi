...
describe('servicios DeepSeek',()=>{
 ...
 test('lista conversaciones',async()=>{const t=new T([{data:{biz_data:{chat_sessions:[{id:'1',title:'T',updated_at:1,model_type:'m'}]}}}]);const ses:any={obtener:async()=>({authorization:'x'})};const r=await new DeepSeekConversaciones(t,ses).listar();expect(r[0]?.id).toBe('1')});
 test('obtiene mensajes',async()=>{const registro={data:{chat_session:{id:'1',title:'T'},chat_messages:[{role:'user',fragments:[{type:'REQUEST',content:'h'}]}]}};const t=new T([registro]);const ses:any={};const r=await new DeepSeekConversaciones(t,ses).mensajes('1');expect(r?.mensajes[0]?.contenido).toBe('h')});
});
