...
describe("proveedores",()=>{
 ...
 test("DeepSeek orquesta página",async()=>{const pagina:any={verificar:async()=>{},abrir:async()=>{},listarModelos:()=>[],seleccionarModelo:async(m:string)=>({id:m,nombre:m}),preparar:async()=>{},enviar:async()=>{},async *observar(){yield{tipo:"respuesta",contenido:"OK"};yield{tipo:"fin"}},modeloActual:async()=>"default"};const conv:any={listar:async()=>[],mensajes:async()=>null};const ses:any={importar:async()=>{}};const ev=await recoger(new ProveedorDeepSeek(pagina,conv,ses).enviarMensaje({prompt:"hola",modelo:"default"}));expect(ev.at(-1).tipo).toBe("fin")});
});

test("DeepSeek continuar observa sin reenviar el prompt", async()=>{
 let enviados=0; const pagina:any={verificar:async()=>{},abrir:async()=>{},listarModelos:()=>[],seleccionarModelo:async(m:string)=>({id:m,nombre:m}),preparar:async()=>{},enviar:async()=>{enviados++},async *observar(){yield{tipo:"fin"}},modeloActual:async()=>"default"};
